...
Co-expression analysis is sensitive to how expression data is prepared. Follow
these steps before computing similarity matrices:

**1. Normalization**: Choose one of the following (do not combine):

- **VST** (DESeq2 `vst()`): Best for RNA-seq count data. Stabilizes variance
  across the expression range. Recommended when you have raw counts.
- **log2(CPM + 1)**: Simpler alternative. Compute counts-per-million first
  (e.g., via `edgeR::cpm()`), then log-transform. Works well for most cases.
- **TPM/FPKM**: Acceptable if already provided, but less robust than
  count-based normalization for co-expression.

Do **not** use rank-based normalization (e.g., quantile normalization across
genes) -- this destroys the variance structure that correlation depends on.

**2. Filter low-expressed genes**: Genes with very low or zero expression across
most samples contribute noise, not signal. A common filter:

```{r, eval=FALSE}
# Keep genes expressed (e.g., > 1 CPM) in at least 20% of samples
keep <- rowSums(cpm(counts) > 1) >= 0.2 * ncol(counts)
expr_matrix <- expr_matrix[keep, ]
```

**3. Remove zero-variance genes**: Genes with identical expression across all
samples have undefined correlation and will produce `NaN` in similarity
matrices.
...
**4. Handle batch effects**: If your samples come from multiple studies or
batches, consider correction with `limma::removeBatchEffect()` or
`sva::ComBat()` **before** computing similarity. Cross-study data is valuable
for co-expression (more diverse conditions), but uncorrected batch effects can
dominate the correlation structure.

**5. Sample diversity**: More diverse conditions improve co-expression
detection. The paper found diminishing returns after ~100--200 diverse samples.
Biological replicates from the same condition add less information than samples
from different tissues, time points, or treatments.

**6. Gene IDs**: Ensure rownames match the gene IDs in your ortholog table
(check with `intersect(rownames(expr_matrix), orthologs$gene_sp1)`).
...
cat("OpenMP available:", coexpr:::has_openmp(), "\n")
cat("Maximum threads:", coexpr:::get_max_threads(), "\n")
```

### Alternative: MI+CLR similarity

The package also provides mutual information with context likelihood ratio
(MI+CLR) normalization as an alternative to PCC+MR. MI captures nonlinear
relationships that Pearson correlation misses, while CLR reduces hub-gene bias.

```{r mi_clr_example, eval=FALSE}
# MI+CLR similarity (requires more samples than PCC+MR)
sim_mi <- calculate_mi_clr(expr_matrix, n_bins = 10, n_cores = 4)
```

**Choosing `n_bins`**: The number of discretization bins controls the
resolution-variance trade-off:

| Samples | Recommended `n_bins` | Rationale |
|---------|---------------------|-----------|
| 30--50  | 5                   | Minimum for stable MI estimates |
| 50--100 | 8--10               | Good balance for most datasets |
| >100    | 10--15              | Finer resolution justified |

Rule of thumb: use at least 3 samples per bin (`n_samples >= 3 * n_bins`).
Too many bins relative to samples produces noisy MI estimates.

**When to prefer MI+CLR over PCC+MR**: MI+CLR can detect nonlinear
co-expression relationships (e.g., switch-like regulation). However, PCC+MR
consistently outperformed MI+CLR in the Gronvold & Hvidsten benchmarks, so
use PCC+MR as the default and MI+CLR for sensitivity analysis or when you
suspect important nonlinear relationships.
...
### Co-expression similarity by ortholog type

The following heatmaps show within-species co-expression similarity for genes
involved in each ortholog relationship type. Conserved co-expression modules
appear as block-diagonal patterns; similar block structure across the two
species indicates preserved regulatory neighborhoods. Genes are ordered by
hierarchical clustering (average linkage on `1 - similarity`).

```{r sim_heatmaps_by_type, fig.width=16, fig.height=9}
# Convert TriSimilarity to full matrices for subsetting
mat_bdis <- as.matrix(sim_bdis)
mat_bsyl <- as.matrix(sim_bsyl)

# Helper: create one co-expression similarity heatmap panel
make_sim_heatmap <- function(mat, genes, title, show_legend = FALSE) {
  genes <- genes[genes %in% rownames(mat)]
  n <- length(genes)
  if (n < 2) {
    return(ggplot() +
      annotate("text", x = 0.5, y = 0.5, label = paste("n =", n)) +
  ...
  d <- as.dist(1 - sub)
  d[is.na(d) | !is.finite(d)] <- 1
  gene_order <- if (n > 2) genes[hclust(d, method = "average")$order] else genes

  # Long format (column-major matches row=g1, col=g2)
  sub_ordered <- sub[gene_order, gene_order]
  df <- data.frame(
    g1 = rep(gene_order, times = n),
    g2 = rep(gene_order, each = n),
    similarity = as.vector(sub_ordered)
  )
  df$g1 <- factor(df$g1, levels = gene_order)
  df$g2 <- factor(df$g2, levels = rev(gene_order))

  p <- ggplot(df, aes(g1, g2, fill = similarity)) +
    geom_raster() +
    scale_fill_gradient2(
      low = "white", mid = "steelblue", high = "darkred",
      midpoint = 0.5, limits = c(0, 1), name = "PCC+MR\nsimilarity"
    ) +
    coord_fixed() +
...
# Build panels for each ortholog type and species
types <- c("1:1", "1:N", "N:1", "N:M")
panels <- list()
col_widths <- numeric(length(types))

for (i in seq_along(types)) {
  tp <- types[i]
  orth <- orthologs_all[orthologs_all$type == tp, ]
  g_bdis <- unique(orth$gene_sp1)
  g_bsyl <- unique(orth$gene_sp2)
  n_bdis <- sum(g_bdis %in% rownames(mat_bdis))
  n_bsyl <- sum(g_bsyl %in% rownames(mat_bsyl))

  panels[[paste0(tp, "_bdis")]] <- make_sim_heatmap(
    mat_bdis, g_bdis,
    paste0(tp, " \u2014 B. distachyon (n=", n_bdis, ")")
  )
  panels[[paste0(tp, "_bsyl")]] <- make_sim_heatmap(
    mat_bsyl, g_bsyl,
    paste0(tp, " \u2014 B. sylvaticum (n=", n_bsyl, ")"),
    show_legend = (i == length(types))
  )
  col_widths[i] <- max(n_bdis, n_bsyl)
}

# Arrange: columns = ortholog types, rows = species
# Column widths proportional to gene count so each heatmap fills its cell
grid.arrange(
  panels[["1:1_bdis"]], panels[["1:N_bdis"]], panels[["N:1_bdis"]], panels[["N:M_bdis"]],
  panels[["1:1_bsyl"]], panels[["1:N_bsyl"]], panels[["N:1_bsyl"]], panels[["N:M_bsyl"]],
  nrow = 2,
  widths = col_widths,
  top = grid::textGrob(
    "Co-expression similarity by ortholog type",
...
**Interpretation**: Each column shows one ortholog relationship type; each row
shows one species. The 1:1 orthologs (leftmost) have the largest matrices and
typically show the clearest block-diagonal structure, reflecting conserved
co-expression modules. Multi-copy ortholog types (1:N, N:1, N:M) involve fewer
genes and may show weaker or more fragmented block structure, consistent with
expression divergence after gene duplication. Similar patterns across the top
and bottom rows indicate that co-expression neighborhoods are preserved between
species.
